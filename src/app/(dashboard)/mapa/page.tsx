...
const MapView = dynamic(() => import('@/components/ui/MapView'), {
  ssr: false,
  loading: () => (
    <div className="h-[300px] md:h-[500px] bg-gray-100 rounded-xl flex items-center justify-center">
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-lg md:text-2xl font-bold text-gray-900 flex items-center gap-2">
              <MapPin className="h-5 w-5 md:h-6 md:w-6 text-indigo-500" />
        ...
        <div className="flex items-center justify-center gap-1 sm:gap-2 bg-white rounded-xl p-2 shadow-sm border border-gray-100">
          <button
          ...
          <input
            ...
            className="flex-1 max-w-[160px] px-3 py-2 text-sm font-medium text-gray-700 bg-gray-50 border-0 rounded-lg text-center cursor-pointer focus:outline-none focus:ring-2 focus:ring-indigo-200"
          />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        ...
        <div className={`lg:col-span-2 ${mobileView === 'list' ? 'hidden lg:block' : ''}`}>
          <Card padding="none" className="overflow-hidden">
            {loading ? (
              <div className="h-[300px] md:h-[500px] flex items-center justify-center bg-gray-50">
                <div className="flex flex-col items-center gap-2">
                  <div className="animate-spin h-8 w-8 border-4 border-indigo-200 border-t-indigo-500 rounded-full"></div>
                  <span className="text-gray-400 text-sm">Cargando visitas...</span>
                </div>
              </div>
            ) : markers.length === 0 ? (
              <div className="h-[300px] md:h-[500px] flex flex-col items-center justify-center bg-gray-50 px-4">
                <MapPin className="h-10 w-10 md:h-12 md:w-12 text-gray-300 mb-3" />
          ...
          {markers.length > 0 && (
            <div className="mt-2 md:mt-3 flex flex-wrap items-center justify-center md:justify-start gap-3 md:gap-4 text-xs md:text-sm text-gray-500">
          ...
          <Card padding="sm" className="md:p-6">
            ...
            {visits.length === 0 ? (
              <div className="text-center py-6 md:py-8">
                <Calendar className="h-8 w-8 md:h-10 md:w-10 text-gray-300 mx-auto mb-2" />
                <p className="text-sm text-gray-500">Sin visitas</p>
              </div>
            ) : (
              <div className="space-y-2 md:space-y-3 max-h-[60vh] lg:max-h-[450px] overflow-y-auto">
                {visits.map((visit, index) => {
                  ...
                  return (
                    <div
                      key={visit.id}
                      className="p-2.5 md:p-3 rounded-xl bg-gray-50 border border-transparent hover:border-gray-200 transition-colors"
                    >
                      <Link href={`/calendario/${visit.id}`} className="block">
                        <div className="flex items-start gap-2 md:gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-1.5 text-xs md:text-sm text-indigo-600 font-medium">
                      ...
                      <div className="flex gap-2 mt-2">
                        {visit.customer?.telefono && (
                          <a
                            ...
                            className="flex-1 flex items-center justify-center gap-1.5 py-2 px-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-medium rounded-lg transition-colors touch-manipulation"
                          >
                        ...
                        {hasLocation && (
                          <a
                            ...
                            className="flex-1 flex items-center justify-center gap-1.5 py-2 px-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-medium rounded-lg transition-colors touch-manipulation"
                          >
          ...
          {visitsWithoutLocation.length > 0 && (
            <Card padding="sm" className="border-amber-200 bg-amber-50/30">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="font-medium text-gray-900 text-sm">Sin ubicación</h3>
              ...
              <div className="space-y-1.5">
                {visitsWithoutLocation.slice(0, 3).map((visit) => (
                  <Link
                    ...
                    className="flex items-center justify-between p-2 rounded-lg bg-white hover:bg-gray-50 text-xs touch-manipulation"
                  >
                    <span className="text-gray-700 truncate">{visit.customer?.nombre}</span>
                    <span className="text-indigo-600 shrink-0 ml-2">Configurar →</span>
                  </Link>
                ))}
