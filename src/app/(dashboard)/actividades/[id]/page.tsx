...
const estadoColors: Record<ActivityStatus, string> = {
  ...
  haciendo: 'bg-blue-100 text-blue-700',
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <h1 className="text-2xl font-bold text-gray-900">{activity.titulo}</h1>
              <span className={cn('px-2 py-0.5 rounded-full text-xs font-medium', estadoColors[activity.estado])}>
            ...
            <div className="space-y-4">
              {activity.descripcion && (
                <div>
                  ...
                  <p className="text-gray-900">{activity.descripcion}</p>
                </div>
              )}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    {activity.es_virtual && activity.enlace_reunion ? (
                      ...
                      <p className="text-gray-900">{activity.ubicacion}</p>
                    )}
                  </div>
                </div>
              )}
              ...
              {activity.notas && (
                <div className="pt-4 border-t border-gray-100">
                  ...
                  <p className="text-gray-700 whitespace-pre-wrap">{activity.notas}</p>
                </div>
              )}
              ...
              {activity.resultado && (
                <div className="pt-4 border-t border-gray-100">
                  ...
                  <p className="text-gray-900">{activity.resultado}</p>
                </div>
              )}
            </div>
          </Card>
                      ...
                      {linkedActivities.map((act) => (
                        ...
                        >
                          <div className="flex items-start justify-between">
                            ...
                            <span className={cn('px-2 py-0.5 rounded-full text-xs font-medium', estadoColors[act.estado])}>
                              {estadoLabels[act.estado]}
                            </span>
                          </div>
                          {act.descripcion && (
                            <p className="text-sm text-gray-600 mt-2 line-clamp-2">{act.descripcion}</p>
                          )}
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="space-y-3">
                      {linkedVisits.map((visit) => (
                        ...
                        >
                          ...
                          {visit.objetivo && (
                            <p className="text-sm text-gray-600 mt-2 line-clamp-2">{visit.objetivo}</p>
                          )}
                        </Link>
                      ))}
                    </div>
                  ...
                  <div key={p.id} className="flex items-center gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {p.estado_confirmacion && (
                        <p className="text-xs text-gray-500 capitalize">{p.estado_confirmacion}</p>
                      )}
                    </div>
                  </div>
                ))}
            ...
            <Card>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-500">Actividades vinculadas</span>
                  <span className="text-sm font-semibold text-gray-900">{linkedActivities.length}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-500">Visitas vinculadas</span>
                  <span className="text-sm font-semibold text-gray-900">{linkedVisits.length}</span>
                </div>
                <div className="flex justify-between items-center">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-500">Visitas completadas</span>
                  <span className="text-sm font-semibold text-green-600">
